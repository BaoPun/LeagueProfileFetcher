...
void ApiProcessor::open_secondary_window(){
    ...
    if(this->has_entered_new_summoner){
        // Set the summoner name, the region, and the tagline for the main window, which is required for the riot id url.
        if(this->main_window.get_main_signal()){
        ...
        // Set the summoner name, the region, and the tagline for the summoner window, which is required for the riot id url.
        else{
 ...
 */
void ApiProcessor::open_champion_window(int champion_id){
    // Hide the summoner window
    this->summoner_profile_window.hide();

    // Process the champion specific info (already done as static data)
    this->champion_window.set_champion_name(this->static_data.get_champion_name_by_key(champion_id));

    // Process images
    if(this->champion_window.get_champion_name() != "Wukong")
        this->process_api_data(QString::fromStdString("https://ddragon.leagueoflegends.com/cdn/" + this->static_data.get_version() + "/data/en_US/champion/" + this->champion_window.get_champion_name().toStdString() + ".json"), 14);
    else
        this->process_api_data(QString::fromStdString("https://ddragon.leagueoflegends.com/cdn/" + this->static_data.get_version() + "/data/en_US/champion/MonkeyKing.json"), 14);
...
void ApiProcessor::process_api_data(QString url, int json_index){
    ...
    connect(this->net_reply, &QNetworkReply::finished, this, [=, this]{ retrieve_data(json_index); });
}
...
void ApiProcessor::process_multiple_api_data(vector<QString> url_list, int offset){
    for(size_t i = 0; i < url_list.size(); i++){
        ...
        connect(this->net_reply, &QNetworkReply::readyRead, this, &ApiProcessor::read_data);
        connect(this->net_reply, &QNetworkReply::finished, this, [=, this]{ retrieve_data(i + 2 + offset); });
...
void ApiProcessor::process_multiple_image_data(){
    ...
    if(this->database != nullptr){
        ...
        if(!mastered_champions.empty()){
            ...
            QString values;
            for(size_t i = 0; i < mastered_champions.size(); i++){
...
void ApiProcessor::add_image_from_api(vector<QImage>& image_urls){
    ...
    if(this->net_reply->error() != QNetworkReply::NoError){
        ...
        if(!new_image.isNull()){
            new_image = new_image.scaled(80, 150, Qt::KeepAspectRatio, Qt::FastTransformation);
...
void ApiProcessor::retrieve_data(int index){
    ...
    // Set up each summoner's match data
        ...
        cout << "NICE " << this->champion_window.get_champion_name().toStdString() << " DATA" << endl;

        // Process the skins, passive ability, and spells here
        // SO that they can be passed onto the ChampionWindow
        QJsonArray skins = QJsonDocument::fromJson(this->data_buffer).object()["data"].toObject()[this->champion_window.get_champion_name()].toObject()["skins"].toArray();
        QJsonObject passive = QJsonDocument::fromJson(this->data_buffer).object()["data"].toObject()[this->champion_window.get_champion_name()].toObject()["passive"].toObject();
        ...
        for(qsizetype i = 1; i < skins.size(); i++){
            ...
            QEventLoop event_loop;
            connect(this->net_reply, &QNetworkReply::readyRead, this, &ApiProcessor::read_data);
            connect(this->net_reply, &QNetworkReply::finished, this, [&skins_image_list, this]{ add_image_from_api(skins_image_list); });
            connect(this->net_reply, &QNetworkReply::finished, &event_loop, &QEventLoop::quit);
            event_loop.exec();
        }
